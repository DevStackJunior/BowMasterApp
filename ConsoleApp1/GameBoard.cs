...
namespace ConsoleApp1
{
    ...
    public class GameBoard
    {
        ...
        /// <summary>
        /// Drawing towers of each player with equal space between console external borders and each tower 
        /// Creates 2 different ways : 
        /// If player A : draws the tower from top to bottom and from left to right
        /// If player B : draws the tower from top to bottom and from right to left
        /// </summary>
        /// <param name="display"> array of pixels (2nd property the console grid can take) </param>
        /// <param name="player">  </param>
        public void DrawTower(Pixel[,] display, Player player)
        {
            int xTowerPosition = 5;

            // First condition establish the tower A drawing
            // The Grid Console Drawing is by default drawing from TOP to BOTTOM -> Decrementing is necessary in y axis to gain height on tower side draw
            // LEFT to RIGHT
            if (player.PlayerNumber == 1)
            {
                //"display.GetLength(0)" catches the total height (lines accumulation) of the window console as reference point
                for (int y = display.GetLength(0) - 1; y < display.GetLength(0) - player.TowerOwned.HeightTower; y--)
                {
                    //"display.GetLength(1)" catches the total width (columns accumulation) of the window console as reference point
                    for (int x = 0 + xTowerPosition; x < player.TowerOwned.WidthTower + xTowerPosition; x++)
                    {
            ...
            {
                //"display.GetLength(0)" catches the total height of the window console as reference point
                for (int y = display.GetLength(0) - 1; y < display.GetLength(0) - player.TowerOwned.HeightTower; y--)
                {
                    //"display.GetLength(1)" catches the total width of the window console as reference point
                    for (int x = (display.GetLength(1) - xTowerPosition); x > display.GetLength(1) - player.TowerOwned.WidthTower + xTowerPosition; x--)
                    {
                        display[y, x].PixelColor = ConsoleColor.White;
                    }
                }
